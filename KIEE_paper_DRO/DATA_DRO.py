...
class optimization_data(object):
    def __init__(self, seed):
        ...
        self.E_EV_min = 0.95*self.E_EV
        self.E_EV_max = 1.05*self.E_EV
    ...
    def DRO_parameter(self, n_scenario, lamb, Type = 'DRO'):
        ...
        if Type == 'RO':
            self.radius = 999999999999999999999999*np.ones(len(self.radius))
        elif Type == 'SO':
    ...
    def generating_sampling_data(self, n_scenario, data):
        ...
        for data_idx in range(len(data)):
            ...
            samples = np.clip(samples, a_min=self.E_EV_min[data_idx], a_max=self.E_EV_max[data_idx]) # samples의 값을 a_min보다 작은 것을 a_min값으로, a_max보다 큰 것을 a_max로 바꾸자.
